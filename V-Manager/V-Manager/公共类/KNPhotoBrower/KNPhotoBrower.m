...
@interface KNPhotoBrower()<UICollectionViewDataSource,UICollectionViewDelegate>{
    ...
    UILabel               *picnameLabel;
}
@property (nonatomic, strong) N6ActionSheetController *actionSheetController;
@end
...
- (void)initializeOperationView{
    ...
    [shareBtn.layer setCornerRadius:3];
    [shareBtn.layer setMasksToBounds:YES];
    [shareBtn setBackgroundColor:[UIColor clearColor]];
    [shareBtn setAlpha:0.4];
    [shareBtn setBackgroundImage:[UIImage imageNamed:@"分享图标"] forState:UIControlStateNormal];
    [shareBtn setFrame:(CGRect){{ScreenWidth - 35 - 15-25-15,20},{25,25}}];
    [shareBtn addTarget:self action:@selector(shareBtnIBAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)initializeBottomView{
    ...
    picnameLabel = [[UILabel alloc] init];
   
    [picnameLabel setBackgroundColor:[UIColor clearColor]];

    picnameLabel.frame=CGRectMake(ScreenWidth/2-100, ScreenHeight-40, 200, 30);
    picnameLabel.textColor=[UIColor whiteColor];
    [picnameLabel setAlpha:0.6];
    
    KNPhotoItems *items = self.itemsArr[self.currentIndex];
    picnameLabel.text=items.picname;
    picnameLabel.textAlignment=NSTextAlignmentCenter;
    
    [self addSubview:picnameLabel];
...
#warning 如果传入的 ActionSheetArr 有下载图片这一选项. 则在这里调用和下面一样的方法 switch.....,如果没有下载图片,则通过代理方法去实现...
        ...
        KNActionSheet *actionSheet = [[KNActionSheet alloc] initWithCancelBtnTitle:nil destructiveButtonTitle:@"删除" otherBtnTitlesArr:@[@"保存图片"] actionBlock:^(NSInteger buttonIndex) {
...
#pragma mark - 右上角 按钮的点击
- (void)shareBtnIBAction{
    __weak typeof(self) weakSelf = self;
    

        KNActionSheet *actionSheet = [[KNActionSheet alloc] initWithCancelBtnTitle:nil destructiveButtonTitle:nil otherBtnTitlesArr:@[@"分享至微信好友",@"分享至朋友圈",@"分享至QQ"] actionBlock:^(NSInteger buttonIndex) {
...
//                               KNPhotoItems *items = _itemsArr[_currentIndex];
//             
//                        UIStoryboard *userStoryboard = [UIStoryboard storyboardWithName:@"RLKnowledge_Storyboard" bundle:nil];
//                        ShareViewController *vc = (ShareViewController *)[userStoryboard  instantiateViewControllerWithIdentifier:@"sid_shareview"];
//                        vc.titleStr=@"";
//                        vc.imageStr=@"";
//                        vc.linkStr=@"";
//                        //vc.coverImage=@"";
//                        vc.descStr=@"";
//                        //文章分享 非根据类型分享
//                        vc.isNormalShare=YES;
//                        
//                        self.actionSheetController = [N6ActionSheetController newInstance];
//                         BuddhaListViewController *cv = (BuddhaListViewController *)[self.superview nextResponder];
//                        vc.actionSheetController = self.actionSheetController;
//                        [self.actionSheetController showViewController:vc
...
- (UIImageView *)tempViewFromSourceViewWithCurrentIndex:(NSInteger)currentIndex{
    ...
    picnameLabel.text=items.picname;
    NSLog(@"%@", picnameLabel.text);

    if([items.sourceView isKindOfClass:[UIImageView class]]){
    //    UIImageView *imgV = items.defaultImage;
      //genzong
        [tempView setImage:items.defaultImage];
    }

...
//    NSLog(@"%@",self.selectedDate);
    ...
    [detailDict setObject:[NSString stringWithFormat:@"%@",[lunarCalendar.Holiday count]>0?[[[lunarCalendar.Holiday objectAtIndex:0] componentsSeparatedByString:@" "] objectAtIndex:0]:@""] forKey:@"Holiday"];
...
-(void)showPreviousMonth {
    ...
    [UIView animateWithDuration:.35
                     animations:^{
                         ...
                         if (hasPreviousDays) {
                             animationView_A.frameX = animationView_B.frameWidth-(kVRGCalendarViewDayHeight+3);
...
-(void)handleSingleTap:(UITapGestureRecognizer *)sender{
    ...
    NSLog(@"handleSingleTap!pointx:%f,y:%f",touchPoint.x,touchPoint.y);
...
- (void)drawRect:(CGRect)rect
{
    ...
    float a=labelCurrentMonth.frameWidth/2;
    float b=self.frame.size.width/2;
    float c=roundf(self.frame.size.width/2 - labelCurrentMonth.frameWidth/2);
...
-(id)init {
    ...
    if (self) {
        ...
        labelCurrentMonth.text = @"2015年10月";
        labelCurrentMonth.font = [UIFont systemFontOfSize:17];
        [labelCurrentMonth sizeToFit];
        ...
        leftSwipeGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(showNextMonth)];
        rightSwipeGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(showPreviousMonth)];
        ...
        [self addGestureRecognizer:singleTap];　//这个可以加到任何控件上,比如你只想响应WebView，我正好填满整个屏幕

...
- (instancetype)init{
    ...
    self.modalBackgroundColor = [UIColor colorWithWhite:1.0 alpha:1.0];
...
- (void)showWithContentView:(UIView *)contentView andAnimated:(BOOL)animated {
    ...
    if(self.closeButtonLocation == KGModalCloseButtonLocationRight){
        ...
        closeFrame =  CGRectMake(CGRectGetWidth(containerView.bounds)-40,-25, 24, 40);        //    closeFrame.origin.y = -40;
        closeButton.frame = closeFrame;
       //    closeButton.frame=CGRectMake(kvw-30,-40, 24, 40);
    }
...
- (instancetype)init{
    ...
    dispatch_once(&once, ^{
       // closeButtonImage = [self closeButtonImage];
        closeButtonImage = [UIImage imageNamed:@"MM-通用-弹出窗-关闭"];
    });
